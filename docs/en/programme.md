---
layout: default
title: Program
---

## {{ page.title }}

---

The detailed program will be published closer to the event.

The Symposium will feature three complementary formats:

- **Oral presentations** (full papers)  
- **Roundtables** (extended abstracts)  
- **Workshops** (interactive sessions)

In addition, the program will include opening and closing remarks, as well as opportunities for networking and informal exchanges.

---

### Final Program

 <h3>Schedule (Draft)</h3>

<table>
  <thead>
    <tr>
      <th>Time</th>
      <th>Thursday, March 12</th>
      <th>Friday, March 13</th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td><strong>08:30–09:00</strong></td>
      <td></td>
      <td>
        <strong>Welcome Breakfast</strong><br>
        (D.726 – Creativity room)
      </td>
    </tr>

    <tr>
      <td><strong>09:00–10:30</strong></td>
      <td></td>
      <td>
        <strong>Workshop #4</strong><br>
        (A.505) Justine-Anne Rowell
      </td>
    </tr>

    <tr>
      <td><strong>10:30–10:45</strong></td>
      <td></td>
      <td><strong>Break</strong> (15 minutes)</td>
    </tr>

    <tr>
      <td><strong>10:45–12:15</strong></td>
      <td></td>
      <td>
        <strong>Parallel sessions</strong><br><br>
        <strong>Roundtable session</strong> (A.551)<br>
        <em>10:45–12:15</em><br><br>

        <strong>Workshop #5</strong> (C.412) Tobie Rodrigue<br>
        <em>10:45–12:15</em><br><br>

        <strong>Presentation Session #5</strong> (C.521)<br>
        Organizations, Work &amp; Identity<br>
        <em>(IDs: 02, 03, 13, 24)</em>
      </td>
    </tr>

    <tr>
      <td><strong>12:15–13:15</strong></td>
      <td></td>
      <td>
        <strong>Lunch</strong> (starts at 12:15)<br>
        (D.726 – Creativity room)
      </td>
    </tr>

    <tr>
      <td><strong>11:30–13:30</strong></td>
      <td>
        <strong>Arrival &amp; distribution of name tags</strong><br>
        (B.307 – Multipurpose room)
      </td>
      <td></td>
    </tr>

    <tr>
      <td><strong>13:30–14:00</strong></td>
      <td>
        <strong>Opening speech</strong><br>
        (B.307)
      </td>
      <td></td>
    </tr>

    <tr>
      <td><strong>14:00–15:30</strong></td>
      <td>
        <strong>Parallel sessions</strong><br><br>
        <strong>Presentation Session #1</strong> (C.403)<br>
        Sustainability, ESG &amp; Institutional Transformations<br>
        <em>(IDs: 01, 10, 15, 21)</em><br><br>

        <strong>Presentation Session #2</strong> (A.451)<br>
        Markets, Consumers &amp; Meaning<br>
        <em>(IDs: 04, 14, 19)</em><br><br>

        <strong>Workshop #1</strong> (C.406) Vicky Mercier<br><br>

        <strong>Workshop #2</strong> (C.412) Anana Bhardwaj &amp; al.
      </td>

      <td>
        <strong>Parallel sessions</strong><br><br>
        <strong>Presentation Session</strong> (C.521)<br>
        Health, Care &amp; Digitalization<br>
        <em>(IDs: 06, 08, 09, 22)</em><br><br>

        <strong>Presentation Session</strong> (C.539)<br>
        Institutions, Governance &amp; Public Action<br>
        <em>(IDs: 12, 20, 25, 26)</em>
      </td>
    </tr>

    <tr>
      <td><strong>15:30–16:00</strong></td>
      <td><strong>Refreshment</strong> (B.307)</td>
      <td><strong>Refreshment</strong> (D.726)</td>
    </tr>

    <tr>
      <td><strong>16:00–17:30</strong></td>
      <td>
        <strong>Parallel sessions</strong><br><br>
        <strong>Presentation Session #3</strong> (C.403)<br>
        AI, Algorithmic Decision-Making &amp; Fairness<br>
        <em>(IDs: 05, 11, 17, 23)</em><br><br>

        <strong>Presentation Session #4</strong> (A.451)<br>
        Innovation, Learning &amp; Spatial / Policy Dynamics<br>
        <em>(IDs: 07, 16, 18)</em><br><br>

        <strong>Workshop #3</strong> (C.406) Paul &amp; Karl’s
      </td>

      <td>
        <strong>Workshops</strong><br><br>
        <strong>Workshop #6</strong> (C.505) Margaux Maurel<br><br>
        <strong>Workshop #7</strong> (C.515) Aymeric Mailler<br><br>
        <strong>Workshop #8</strong> (C.551) Marie Claire Hamel
      </td>
    </tr>

    <tr>
      <td><strong>17:00–17:30</strong></td>
      <td></td>
      <td>
        <strong>Closing speeches</strong><br>
        (A.878)
      </td>
    </tr>

    <tr>
      <td><strong>17:30–20:30</strong></td>
      <td>
        <strong>Cocktail</strong> + speech from Denis<br>
        (A.878 – Guy-Fréchette)
      </td>
      <td>
        <strong>Cocktail</strong><br>
        (A.878 – Guy-Fréchette)
      </td>
    </tr>

  </tbody>
</table>

---

*The finalized schedule, session assignments, and presentation times will be announced after the review process is completed.*